...
def run(context, url):
    ...
    for media_server in servers:
        ...
        if content_type == 'video':
            ...
            url = '%s/%s' % (media_server.get_url_location().rstrip('/'), 'video')
            if servers_list == 1:
            ...
            url = '%s/%s' % (media_server.get_url_location().rstrip('/'), 'system/plexonline')
            if servers_list == 1:
            ...
            url = '%s/%s' % (media_server.get_url_location().rstrip('/'), 'music')
            if servers_list == 1:
            ...
            url = '%s/%s' % (media_server.get_url_location().rstrip('/'), 'photos')
            if servers_list == 1:
