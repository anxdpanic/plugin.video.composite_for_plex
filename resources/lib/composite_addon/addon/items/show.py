...
def create_show_item(context, item, library=False):
    ...
    if context.settings.flatten_seasons() == '2':
        ...
        item_url = '%s/%s' % (item.server.get_url_location().rstrip('/'),
                              extra_data['key'].replace('children', 'allLeaves').lstrip('/'))
    else:
        extra_data['mode'] = MODES.TVSEASONS
        item_url = '%s/%s' % \
                   (item.server.get_url_location().rstrip('/'), extra_data['key'].lstrip('/'))
